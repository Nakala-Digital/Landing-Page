        ...
        <details class="lg:hidden relative">
            ...
            <div class="absolute right-0 mt-3 w-56 rounded-xl bg-white shadow-lg ring-1 ring-black/5 p-2">
                @foreach ($navItems as $item)
                    ...
                    @endphp
                    <a
                        class="block rounded-lg px-4 py-2.5 font-button text-button uppercase tracking-wider transition-colors {{ $active ? 'bg-primary/10 text-primary' : 'text-on-surface-variant hover:bg-slate-50 hover:text-primary' }}"
                        href="{{ route($item['route'] . $localeSuffix) }}"
                    >
                        {{ $item['label'] }}
                    </a>
                ...
                <div class="mt-2 pt-2 border-t border-slate-100 flex items-center justify-around text-sm font-semibold p-2">
