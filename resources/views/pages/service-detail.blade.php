...
<body class="bg-background text-on-background font-body-md">
    ...
    <main class="pt-20">
        ...
        <section id="hero" class="relative overflow-hidden bg-gradient-to-br from-surface-container-lowest via-surface to-primary/5 pt-unit-xl pb-unit-xl border-b border-outline-variant z-0">
            ...
            <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop relative z-10">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-unit-xl items-center relative">
                    ...
                    <div class="lg:col-span-6 relative z-10 hidden lg:block">
                        <div class="relative rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-outline-variant/30">
                            <img src="{{ $p['hero_image'] }}" alt="{{ $p['title'][$locale] }}" class="w-full h-auto object-cover" loading="lazy">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="px-margin-mobile md:px-margin-desktop py-unit-xl max-w-container-max mx-auto">
            ...
            {{-- Figma mirrored pattern: cyan, lime, navy | navy, lime, cyan --}}
            @php
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-unit-md">
                @foreach ($p['capabilities'] as $index => $cap)
                    <div class="bg-surface-container-low p-unit-lg rounded-xl border-l-8 {{ $mirroredPattern[$index % count($mirroredPattern)] }}">
        ...
        <section class="px-margin-mobile md:px-margin-desktop py-unit-xl max-w-container-max mx-auto">
            ...
            @php
                $consulting = [
                    ['title' => ['en' => 'Technology Consulting', 'id' => 'Konsultasi Teknologi'], 'desc' => ['en' => 'Helping organizations align digital transformation strategy, design solution architecture, and select the right technology to achieve business goals.', 'id' => 'Membantu organisasi menyesuaikan strategi transformasi digital, merancang arsitektur solusi, dan memilih teknologi yang tepat untuk mencapai tujuan bisnis.']],
                    ['title' => ['en' => 'Experience-Led Design', 'id' => 'Desain Berpusat pada Pengalaman Pengguna'], 'desc' => ['en' => 'Designing solutions by understanding user needs through research, wireframes, and interfaces focused on user requirements.', 'id' => 'Merancang solusi dengan memahami kebutuhan pengguna melalui riset, wireframe, dan desain antarmuka yang berfokus pada kebutuhan pengguna.']],
                    ['title' => ['en' => 'AI Readiness Assessment', 'id' => 'Evaluasi Kesiapan AI'], 'desc' => ['en' => 'Evaluating organizational readiness to adopt AI and identifying automation opportunities that deliver business value.', 'id' => 'Mengevaluasi kesiapan organisasi dalam mengadopsi AI serta mengidentifikasi peluang otomasi yang memberikan nilai bisnis.']],
                ];
                $build = [
                    ['title' => ['en' => 'Custom Software Development', 'id' => 'Pengembangan Perangkat Lunak Kustom'], 'desc' => ['en' => 'Building web, mobile, SaaS, and enterprise applications that are secure, flexible, and easy to develop.', 'id' => 'Membangun aplikasi web, mobile, SaaS, dan sistem enterprise yang aman, fleksibel, dan mudah dikembangkan.']],
                    ['title' => ['en' => 'AI & Generative AI Platform', 'id' => 'Platform AI & Generative AI'], 'desc' => ['en' => 'Building AI and generative AI-based platforms that integrate, process documents, and improve business productivity.', 'id' => 'Membangun platform berbasis AI dan generative AI yang terintegrasi, memproses dokumen, dan meningkatkan produktivitas bisnis.']],
                    ['title' => ['en' => 'Operational Systems', 'id' => 'Sistem Operasional'], 'desc' => ['en' => 'Developing operational systems that automate business processes more efficiently and in an integrated manner.', 'id' => 'Mengembangkan sistem operasional yang mengotomatisasi proses bisnis secara lebih efisien dan terintegrasi.']],
                ];
                $run = [
                    ['title' => ['en' => 'QA / Quality Assurance Governance', 'id' => 'Tata Kelola Jaminan Kualitas (QA) / Tata Kelola QA'], 'desc' => ['en' => 'Implementing software governance through comprehensive testing, QA governance, and structured release processes.', 'id' => 'Mengimplementasikan tata kelola perangkat lunak melalui pengujian menyeluruh, tata kelola QA, dan proses rilis yang terstruktur.']],
                    ['title' => ['en' => 'Managed Support Services', 'id' => 'Layanan Dukungan Terkelola'], 'desc' => ['en' => 'Providing support and maintenance services to ensure business systems run optimally after implementation.', 'id' => 'Menyediakan layanan dukungan dan pemeliharaan untuk memastikan sistem bisnis tetap berjalan optimal setelah implementasi.']],
                    ['title' => ['en' => 'Microsoft Dynamics 365 & Power Platform', 'id' => 'Microsoft Dynamics 365 & Power Platform'], 'desc' => ['en' => 'Implementation of Microsoft Dynamics 365 and Power Platform integration to improve business productivity and efficiency.', 'id' => 'Implementasi Microsoft Dynamics 365 dan integrasi Power Platform untuk meningkatkan produktivitas dan efisiensi bisnis.']],
                ];
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-unit-md">
                ...
                @foreach ($headers as $h)
                    <div class="{{ $h['bg'] }} {{ $h['text'] }} text-center py-4 rounded-t-xl font-semibold">
                        {{ $h['label'] }}
                    </div>
                @endforeach
                ...
                @for ($row = 0; $row < 3; $row++)
                    @foreach ($columns as $colIndex => $col)
                        <div class="bg-surface-container-low p-unit-md rounded-lg border-l-8 {{ $colBorders[$colIndex] }}">
                            <h4 class="font-semibold text-on-background mb-1">{{ $col[$row]['title'][$locale] }}</h4>
                            <p class="text-sm text-on-surface-variant">{{ $col[$row]['desc'][$locale] }}</p>
                        </div>
                    @endforeach
                @endfor
            </div>
        ...
        <section class="bg-surface-container-low px-margin-mobile md:px-margin-desktop py-unit-xl">
            <div class="max-w-container-max mx-auto">
                ...
                {{-- Heading --}}
                <h2 class="font-headline-h2 text-3xl md:text-headline-h2 text-on-background mb-unit-lg text-center">
                    ...
                    @foreach ($caseStudies as $case)
                        <a href="{{ route('solutions.detail' . $localeSuffix, ['solution' => $case['id']]) }}"
                            ...
                            <div class="p-unit-lg flex flex-col flex-1">
                                {{-- Category Badge --}}
                                <span class="inline-block bg-surface-container-low text-primary text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-unit-sm self-start">
                ...
                <div class="flex justify-end">
                    <a href="{{ route('portfolio' . $localeSuffix) }}"
                        class="inline-flex items-center gap-2 bg-midnight-navy text-white px-unit-lg py-3 rounded-[15px] font-button text-button uppercase tracking-wider hover:opacity-90 transition-opacity">
        ...
        <section class="bg-electric-cyan px-margin-mobile md:px-margin-desktop py-unit-xl">
            <div class="max-w-container-max mx-auto text-center">
                <h2 class="font-headline-h2 text-3xl md:text-headline-h2 text-white mb-unit-md">
                ...
                <a href="{{ route('contact' . $localeSuffix) }}"
                    class="inline-flex items-center justify-center gap-2 bg-impact-lime text-midnight-navy px-unit-lg py-3 rounded-[20px] font-button text-button uppercase tracking-wider hover:opacity-90 transition-opacity">
